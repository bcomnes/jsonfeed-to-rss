...
function getSummary (obj, opts = {}) {
  return truncate4k(
    get(obj, '_itunes.summary') ||
    get(opts, 'itunes.summary') ||
    get(obj, 'summary') ||
...
function getSubtitle (obj, opts = {}) {
  return truncate250(
    get(obj, '_itunes.subtitle') ||
    get(opts, 'itunes.subtitle') ||
...
function getPodcastType (jf, opts) {
  return ['episodic', 'serial'].some(type => {
    const possibleType = get(jf, '_itunes.type') || get(opts, 'itunes.type')
    return possibleType === type
  }) ? get(jf, '_itunes.type') : 'episodic'
}
