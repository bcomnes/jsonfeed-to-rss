...
module.exports = function generateTitle (item) {
  ...
  if (item.summary) return truncate(getFirstParagraph(item.summary))
  if (item.content_text) { return truncate(getFirstParagraph(item.content_text)) }
  if (item.content_html) return truncate(getFirstParagraph(striptags(item.content_html)))
